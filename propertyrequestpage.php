            ...
            <table class="items-list col-lg-12 table-hover" style="border-collapse:inherit;">
              <thead>
                <tr class="bg-dark">
                  ...
                  <th class="text-white font-weight-bolder">Buyer</th>
              ...
              <tbody>
                ...
                <?php
                $select_requests = "SELECT * FROM `request` WHERE receiver = '$user_id'";
                ...
                if (mysqli_num_rows($result_requests) > 0) {
                  while ($fetch_request = mysqli_fetch_assoc($result_requests)) {
                    ...
                    $select_sender = "SELECT * FROM `user` WHERE uid = '" . $fetch_request['sender'] . "'";
                    $result_sender = mysqli_query($con, $select_sender);
                ...
                ?>
                    ...
                    <tr>
                      ...
                      <td><?= $fetch_sender['uname']; ?></td>
                      
                      <td><?= $fetch_request['date']; ?></td>
                      
                      <td>
                        <!-- <a class="btn btn-info" href="submitpropertyupdate.php?id=<?php echo $row['0']; ?>">Accept</a> -->
                      
                        <?php
												if ($fetch_request['status'] == "1") {
                          echo '<button class="btn btn-success" onclick="togleaccept('.$fetch_request['id'].');">Accepted</button>';
												} else {
                          echo '<button class="btn btn-info" onclick="togleaccept('.$fetch_request['id'].');">Accept</button>';
												}
												?>
                        <input type="hidden" name="request_id" value="<?= $fetch_request['id']; ?>">
                      </td>
                     <td> <input type="submit" value="Delete Request" class="btn btn-danger" onclick="return confirm('remove this request?');" name="delete"></td>
...
function togleaccept(id) {
	var id = id;
	$.ajax({
		url: "request_aproval.php",
		type: "POST",
		data: {
			uid: id
		},
